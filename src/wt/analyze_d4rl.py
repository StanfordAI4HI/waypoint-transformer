...
from wt import policies, step, util, visualize
...
def run_reward_conditioning(
    ...
    if 'hopper' in env.unwrapped.spec.id:
        ...
        if 'medium-expert' in env.unwrapped.spec.id:
            ...
            # reward_fractions = [0.88, 0.89] #goalnet
        else:
            reward_fractions = [0.85, 0.8, 0.825, 0.875, 0.90, 0.925, 0.95] #goalnet
            # reward_fractions = [0.825, 0.875, 0.85, 0.9, 0.925, 0.95] #no goalnet

    # reward_fractions = [0.3, 0.28, 0.29, 0.31, 0.32]
...
if __name__ == "__main__":
    ...
    run_dir = args.run_id
    out_directory, wandb_run = f'{run_dir}/files', None# util.resolve_out_directory(args.run_id, args.entity)
